...
class Block(nn.Module):
    def __init__(self, args, i):
        ...
        self.sa_list = nn.ModuleList([SA(args) for _ in range(self.num)])
        self.sga_list = nn.ModuleList([SGA(args) for _ in range(self.num)])
    ...
    def forward(self, x, x_mask, y, y_mask):
        ...
        # the best parameters: use co-attention map twice

        # for i in range(self.num):
        #     ax = self.sa_list[i](x,x_mask)
        #     ay = self.sga_list[i](y, x, y_mask, x_mask)
        #     x = ax + x
        #     y = ay +y

        for (sa,sga) in zip(self.sa_list,self.sga_list):
            ax = sa(x,x_mask)
            ay = sga(y,x,y_mask,x_mask)
            x = ax + x
            y = ay + y
...
class Model_LA_stack(nn.Module):
    def __init__(self, args, vocab_size, pretrained_emb):
        super(Model_LA_stack, self).__init__()
